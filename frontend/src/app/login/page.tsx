...
export default function LoginPage() {
    ...
    // Redirect legacy links that land on /login
    useEffect(() => {
        if (typeof window !== "undefined") {
            ...
            if (mode && oobCode) {
                ...
                // Next.js router.push() handles relative paths fine
                window.location.href = `/auth/action?mode=${mode}&oobCode=${oobCode}`;
            }
            if (params.get("verified") === "true") {
                setInfo("Email verified successfully! Please sign in.");
                setShowEmailForm(true); // Auto-show the form for convenience
            }
        }
    }, []);
        ...
        try {
            ...
            if (user) {
                await user.sendEmailVerification({
                    url: `${window.location.origin}/auth/action`, // Direct to action handler
                });
            }
    ...
    const handleWhopLogin = async () => {
        try {
            ...
            await startWhopOAuth(clientId, redirectUri, "openid profile email");
        } catch (e: any) {
            console.error("Whop login error", e);
            setError(e.message);
    ...
    return (
        <div className="fixed inset-0 flex items-center bg-black">
            ...
            <div className="relative z-10 flex flex-col items-center gap-6 ml-[8%] md:ml-[12%] w-full max-w-sm">
                ...
                {/* Google & Whop Sign-in Buttons - side by side */}
                <div className="flex w-full items-center gap-3">
                    <button
                        type="button"
                        onClick={handleGoogleLogin}
                        disabled={loading}
                        className="glass-button flex-1 py-4 text-base font-medium text-white/90 tracking-wide hover:text-white transition-colors"
                    >
                        {loading ? (
                    ...
                    >
                        {whopLoading ? (
                            ...
                            <div className="flex items-center justify-center gap-2">
                                <span className="text-sm">sign in with</span>
                                <img src="/images/whoplogo.png" alt="Whop" className="h-4 w-auto" />
                            </div>
                        )}
                    </button>
                </div>
