...
class WhopOAuthRequest(BaseModel):
    access_token: str


@router.post("/oauth")
async def whop_oauth(body: WhopOAuthRequest):
    """
    Exchange Whop OAuth Access Token for Firebase Custom Token.
    Used for "Login with Whop" flow from the standalone login page.
    
    1. Call Whop /oauth/userinfo to get user info
    2. Check Roles via check_access
    3. Sync User to DB
    4. Mint Firebase Custom Token
    """
    try:
        logger.info("Whop OAuth: Starting OAuth token exchange")
        
        # 1. Call Whop userinfo endpoint
        async with httpx.AsyncClient() as client:
            ...
            if userinfo_resp.status_code != 200:
                logger.error(f"Whop OAuth: userinfo failed with {userinfo_resp.status_code}")
                raise HTTPException(status_code=401, detail="Invalid OAuth token")
            
            userinfo = userinfo_resp.json()
        
        whop_user_id = userinfo.get("sub")
        if not whop_user_id:
        ...
        logger.debug(f"Whop OAuth: Got user {whop_user_id}, email={email}, verified={email_verified}")
        ...
        logger.info(f"Whop OAuth: Role '{role}' determined for {whop_user_id}")
        
        # 3. Sync User (same logic as whop_exchange)
        firebase_uid = f"whop:{whop_user_id}"
        ...
        async with get_db_connection() as conn:
            ...
            if row:
                user_uuid = row[0]
                logger.debug(f"Whop OAuth: Updating existing user {user_uuid}")
                await conn.execute(
                    text("UPDATE users SET role = :role WHERE id = :uid"),
                    {"role": role, "uid": user_uuid}
                )
            else:
                logger.info(f"Whop OAuth: Creating new user for {whop_user_id}")
                result = await conn.execute(
                    text("""
                        INSERT INTO users (firebase_uid, role, whop_user_id, current_period_start)
                        VALUES (:fuid, :role, :wuid, NOW())
        ...
        try:
            try:
                fb_auth.update_user(firebase_uid, photo_url=photo_url, display_name=username)
            except fb_auth.UserNotFoundError:
                fb_auth.create_user(uid=firebase_uid, photo_url=photo_url, display_name=username, email=email)
            except Exception as e:
                logger.warning(f"Firebase User Update failed: {e}")
        except Exception:
            pass
        ...
        logger.info(f"Whop OAuth: Successfully minted token for {whop_user_id}")
        return {
            "firebase_custom_token": custom_token.decode("utf-8"),
            "role": role
        }
        ...
        logger.exception(f"Whop OAuth Error: {e}")
        raise HTTPException(status_code=500, detail="Internal Auth Error")
