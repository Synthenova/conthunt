"""TikTok Top Search platform adapter."""
...
class TikTokTopAdapter:
    """Adapter for TikTok top search via ScrapeCreators API."""
    ...
    async def fetch(
        ...
        """Fetch TikTok top search results."""
        settings = get_settings()
        
        request_params = {
            "query": query,
            "publish_time": params.get("publish_time", "all"),
        ...
        response = await client.get(
            f"{settings.SCRAPECREATORS_BASE_URL}/v1/tiktok/search/top",
    ...
    def parse(self, response_json: dict) -> ParsedPlatformResponse:
        """Parse TikTok top search response into normalized items."""
        items: list[NormalizedItem] = []
        
        # TikTok top response structure: items[] with video and statistics
        ...
        for video_item in video_items:
            ...
            # AI dynamic cover (animated thumbnail)
            ai_cover_urls = video_obj.get("ai_dynamic_cover", {}).get("url_list", [])
            if ai_cover_urls:
                media_urls.append(MediaUrl(
                    asset_type=AssetType.COVER,
                    source_url=ai_cover_urls[0],
                    source_url_list=ai_cover_urls,
                ))
            
            # Regular cover
            cover_urls = video_obj.get("cover", {}).get("url_list", [])
            if cover_urls and cover_urls != ai_cover_urls:
                media_urls.append(MediaUrl(
            ...
            # Get external ID
            external_id = str(video_item.get("aweme_id") or video_item.get("id", ""))
            if not external_id:
                continue
...
tiktok_top_adapter = TikTokTopAdapter()
